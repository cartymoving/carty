...
export default async function Section2(){
  return(
    <section className='section3 w-screen h-fit min-h-screen skew-y-[-14deg] md:skew-y-[-7deg] translate-y-40'>
      <div className='bg-mygreen w-[14vw] h-8 absolute -top-8 left-0'></div>
      <div className='bg-mygreen w-[14vw] h-8 absolute -bottom-8 right-0'></div>
      <div className='w-screen relative h-fit skew-y-[14deg] md:skew-y-[7deg] translate-y-40 px-[10vw]'>
        <div className='flex gap-[7.5vw] relative'>
          <div className='flex flex-col w-[calc(50%-3.75vw)]'>
            <p className='text-mygreen font-bold font-Hind text-2xl'>About Us</p>
            <h1 className='text-mywhite font-bold font-Montserrat text-[4rem] mt-4 mb-12'>Why Carty Movers Is Your Best Choice</h1>
            <p className='text-mywhite font-Hind font-medium text-2xl mb-16'>At Carty Moving Company, we don’t just move boxes; we move lives. Our mission is to provide moving services that cater to your needs, ensuring a smooth transition to your new location. Trust us to handle your move with care, efficiency, and professionalism.</p>
            <Link href="#" className='quote w-fit lg:flex hidden gap-2 px-4 py-2 xl:py-[15px] xl:px-7 bg-mygreen mr-4 hover:bg-mywhite border-mygreen border-2 rounded-md font-Hind text-mywhite hover:text-mygreen font-bold transition-all'>
          ...
          <div className='w-[calc(50%-3.75vw)] h-fit rounded-lg relative'>
            <Image src="/sec3_1.png" width="679" height="529" alt='Worker help' unoptimized={true} className='w-full h-full object-cover rounded-lg'/>
          </div>
        </div>
        <div></div>
      </div>
